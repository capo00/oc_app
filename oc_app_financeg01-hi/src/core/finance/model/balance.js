...
class Account {
  ...
  getTransactions(from = null, to = null) {
    ...
    to = to || (from ? new UuDate() : null);

    let transactions = this.transactions.filter((tx) => {
      let con = (!from && !to) || (UuDate.compare(from, tx.date) <= 0 && UuDate.compare(tx.date, to) <= 0);
...
function parseDate(date, loc) {
  return UuDate.parse(date, loc).toIsoString();
}
...
export default class Balance {
    ...
    data.forEach((row) => {
      if (row.length > i.sc + 1) {
        ...
        let tx = new Transaction({
          ...
          account: /^\d+(?:-\d*)?/.test(row[i.accountNumber].trim())
  ...
  static _getAirTx(data) {
    data.shift();

    const i = CSV_INDEXES.air;
    ...
    data.forEach((row) => {
      if (row.length > 1) {
        ...
        let tx = new Transaction({
          ...
          value: value,
          currency: row[i.currency],
          date: parseDate(row[i.date] || row[i.dateTemp], "en-gb"),
